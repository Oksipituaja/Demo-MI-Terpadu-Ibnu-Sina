                ...
                @forelse($agendas as $agenda)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">
                            <div>{{ $agenda->event_date->format('d M Y') }}</div>
                        ...
                        <td class="px-6 py-4">
                            @php
                                $statusMap = [
                                    'upcoming'  => ['label' => 'Mendatang',   'class' => 'bg-blue-100 text-blue-700'],
                                    'ongoing'   => ['label' => 'Berlangsung', 'class' => 'bg-yellow-100 text-yellow-700'],
                                    'completed' => ['label' => 'Selesai',     'class' => 'bg-gray-100 text-gray-600'],
                                ];
                                $s = $statusMap[$agenda->status] ?? ['label' => ucfirst($agenda->status), 'class' => 'bg-gray-100 text-gray-600'];
                            @endphp
                            <span class="px-2.5 py-1 rounded-full text-xs font-medium {{ $s['class'] }}">
                                {{ $s['label'] }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
