...
<div class="max-w-2xl p-6 mx-auto bg-white rounded-lg shadow">
    <form action="{{ route('admin.prestasis.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        {{-- Slug hidden — auto-generate dari judul --}}
        ...
        <div>
            ...
            <textarea name="description" rows="4" required
                ...
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
        ...
        <div>
            ...
            <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
                ...
                <option value="published" {{ old('status') === 'published' ? 'selected' : '' }}>Publikasi (Tayang)</option>
            </select>
            @error('status') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
        </div>

        <div class="flex gap-3 pt-4 border-t">
            @include('components.admin-submit-btn', ['label' => 'Simpan Prestasi', 'loading' => 'Menyimpan...'])
