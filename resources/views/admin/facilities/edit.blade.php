...
<div class="max-w-2xl p-6 mx-auto bg-white rounded-lg shadow">
    <form action="{{ route('admin.facilities.update', $facility) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        @csrf @method('PUT')
        ...
        <div>
            ...
            <p class="mt-1 text-xs text-gray-500">
                Cari ikon di <a href="https://fontawesome.com/icons" target="_blank" class="text-blue-600 hover:underline">fontawesome.com/icons</a>
            </p>
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description">{{ old('description', $facility->description) }}</textarea>
        </div>

        <div>
            <label class="block mb-1 text-sm font-medium text-gray-700">Kondisi Fasilitas</label>
            <select name="kondisi" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
...
document.addEventListener('DOMContentLoaded', function () {
    window.updateIconPreview = function () {
        ...
        document.getElementById('iconPreviewEl').className = (val || 'fas fa-question') + ' text-blue-600 text-lg';
    };
