        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ \App\Models\User::where('is_active', true)->get()->filter(function($u) { return in_array($u->role->value, ['admin', 'super_admin']); })->count() }}</p>
                </div>
...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('admin.management-account.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium">
            ...
            @forelse($users as $user)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm">
                        ...
                        @endphp
                        <span class="inline-block px-3 py-1 text-xs font-semibold rounded-full {{ $roleBadgeClass }}">
                            {{ $roleLabel }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-sm">
                    ...
                    <td class="px-6 py-4 text-sm space-x-2">
                        <a href="{{ route('admin.management-account.edit', $user) }}" class="text-blue-600 hover:text-blue-800">
                            <i class="fas fa-edit"></i> Edit
                        </a>
                        <form action="{{ route('admin.management-account.destroy', $user) }}" method="POST" style="display:inline;" onsubmit="return confirm('Hapus pengguna ini?')">
                            @csrf @method('DELETE')
