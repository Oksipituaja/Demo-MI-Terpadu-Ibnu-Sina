...
<div class="max-w-2xl p-6 mx-auto bg-white rounded-lg shadow">
    <form action="{{ route('admin.about.update', $about) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        {{-- School Info Fields (only shown when key = school_info) --}}
        @php
            $schoolInfo = $about->key === 'school_info' ? json_decode($about->content, true) : [];
        @endphp
        <div id="schoolInfoFields" class="{{ $about->key === 'school_info' ? '' : 'hidden' }} space-y-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div id="imageField">
            ...
            @if($about->image)
                <div class="p-4 mb-4 border border-gray-200 rounded-lg bg-gray-50">
                    ...
                    <img src="{{ asset('storage/' . $about->image) }}" alt="{{ $about->title }}" class="object-cover h-40 max-w-sm rounded">
                </div>
            @endif
            <div class="p-6 text-center transition border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:border-blue-500 hover:bg-blue-50" id="dropZone">
                ...
                <div>
                    ...
                    <p class="text-gray-600">Drag & drop atau <button type="button" class="font-medium text-blue-600 hover:text-blue-700" onclick="document.getElementById('image').click()">pilih file</button></p>
        ...
        <div class="flex gap-3 pt-4 border-t">
            ...
            <a href="{{ route('admin.about.index') }}" class="px-4 py-2 text-gray-800 bg-gray-200 rounded-lg">Cancel</a>
        </div>
    </form>
</div>
