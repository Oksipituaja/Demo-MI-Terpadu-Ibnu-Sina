...
<div class="max-w-2xl p-6 mx-auto bg-white rounded-lg shadow">
    <form action="{{ route('admin.about.update', $about) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        @php
            $info = [];
            if ($about->key === 'school_info') { $info = json_decode($about->content, true) ?: []; }
        ...
        <div id="contentWrapper" class="{{ $about->key === 'school_info' || in_array($about->key, $imageOnlyKeys) ? 'hidden' : '' }}">
            ...
            <div id="tinymce-skeleton" class="w-full overflow-hidden bg-gray-100 border border-gray-200 rounded-lg" style="height:350px;">
                <div class="flex items-center gap-2 px-3 py-2 border-b border-gray-200 bg-gray-50">
                    @for ($i = 0; $i < 10; $i++)
                        <div class="h-5 bg-gray-200 rounded animate-pulse" style="width:{{ [28,28,32,28,32,28,28,36,28,32][$i] }}px"></div>
                        @if(in_array($i,[1,3,6]))<div class="w-px h-5 mx-1 bg-gray-300"></div>@endif
                    @endfor
                </div>
        ...
        {{-- ✅ Crop Components — dua varian, JS yang mengatur tampilnya --}}
        <div id="imageFieldWrapper" class="{{ in_array($about->key, $noImageKeys) ? 'hidden' : '' }}">
            <div id="crop169" class="{{ $about->key === 'principal_greeting' ? 'hidden' : '' }}">
                <x-image-crop-upload
                    ...
                    aspect-ratio="16/9"
                    :optional="true"
                    :current-image="$about->featured_image && $about->key !== 'principal_greeting'
                        ? asset('storage/' . $about->featured_image) : null"
                    :current-alt="$about->title"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function buildJson() {
        const data = { npsn: document.getElementById('si_npsn').value, nama_sekolah: document.getElementById('si_name').value };
    ...
    function initTinyMCE() {
        ...
        tinymceReady = true;
        tinymce.init({
            selector: '#contentField', license_key: 'gpl', height: 350, menubar: false,
    ...
    function toggleFields() {
        ...
        if (isSchoolInfo) {
            if (typeof tinymce !== 'undefined' && tinymce.get('contentField')) {
                tinymce.get('contentField').remove(); tinymceReady = false;
            }
