        ...
        <form action="{{ route('admin.about.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <select id="keySelect" name="key" required
                    ...
                    <option value="hero_image" {{ old('key') === 'hero_image' ? 'selected' : '' }}>Gambar Utama
                        (Homepage)</option>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="text" id="si_alamat"
                            value="Jl. Raya Bangsri - Keling KM.4, Dukuh Segawe, Desa Jinggotan, Kec. Kembang, Kab. Jepara 59457"
                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
            </div>
...
{{-- FIXED: @push must be OUTSIDE @section, not inside it (was @endsection at EOF before @push) --}}
@push('scripts')
    <script>
