<!DOCTYPE html>
<html lang="id">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pertanyaan Diterima &#8212; MI Terpadu Ibnu Sina</title>
    <style>
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }

        body {
            font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, Arial, sans-serif;
            background-color: #edf2e9;
            color: #374151;
            -webkit-font-smoothing: antialiased;
        }

        .email-wrapper {
            max-width: 600px;
            margin: 32px auto;
            background: #ffffff;
            border-radius: 20px;
            overflow: hidden;
            box-shadow: 0 4px 40px rgba(21, 128, 61, .12);
        }

        .header {
            background: linear-gradient(135deg, #14532d 0%, #15803d 55%, #22c55e 100%);
            padding: 36px 40px 32px;
        }

        .header-top {
            display: flex;
            align-items: center;
            gap: 14px;
            margin-bottom: 18px;
        }

        .logo-box {
            width: 52px;
            height: 52px;
            border-radius: 12px;
            background: #ffffff;
            display: flex;
            align-items: center;
            justify-content: center;
            flex-shrink: 0;
            padding: 4px;
            box-shadow: 0 2px 10px rgba(0, 0, 0, .20);
        }

        .logo-box img {
            width: 44px;
            height: 44px;
            object-fit: contain;
            border-radius: 8px;
            display: block;
        }

        .header-school-name {
            color: rgba(255, 255, 255, .92);
            font-size: 14px;
            font-weight: 700;
            line-height: 1.3;
        }

        .header-school-sub {
            display: block;
            font-size: 11px;
            font-weight: 500;
            color: rgba(255, 255, 255, .58);
            margin-top: 3px;
        }

        .header-divider {
            height: 1px;
            background: rgba(255, 255, 255, .14);
            margin-bottom: 22px;
        }

        .header h1 {
            color: #ffffff;
            font-size: 22px;
            font-weight: 800;
            margin-bottom: 6px;
            letter-spacing: -.3px;
        }

        .header-subtitle {
            color: rgba(255, 255, 255, .62);
            font-size: 13px;
        }

        .status-badge {
            display: inline-flex;
            align-items: center;
            gap: 7px;
            background: rgba(255, 255, 255, .14);
            border: 1px solid rgba(255, 255, 255, .22);
            color: #ffffff;
            font-size: 11px;
            font-weight: 700;
            padding: 6px 14px;
            border-radius: 999px;
            margin-top: 16px;
            letter-spacing: .05em;
            text-transform: uppercase;
        }

        .badge-dot {
            width: 7px;
            height: 7px;
            border-radius: 50%;
            background: #4ade80;
            flex-shrink: 0;
        }

        .body {
            padding: 32px 40px 36px;
        }

        .greeting {
            font-size: 15px;
            color: #374151;
            margin-bottom: 28px;
            line-height: 1.75;
        }

        .greeting strong {
            color: #15803d;
            font-weight: 700;
        }

        .section-label {
            font-size: 10px;
            font-weight: 800;
            text-transform: uppercase;
            letter-spacing: .14em;
            color: #9ca3af;
            margin-bottom: 10px;
        }

        .question-subject {
            font-size: 13px;
            font-weight: 700;
            color: #14532d;
            margin-bottom: 8px;
            padding-left: 4px;
        }

        .question-box {
            background: #f8fdf9;
            border-left: 3px solid #bbf7d0;
            border-radius: 0 10px 10px 0;
            padding: 14px 18px;
            margin-bottom: 24px;
        }

        .question-box p {
            font-size: 14px;
            color: #6b7280;
            line-height: 1.75;
        }

        .info-box {
            background: #f0fdf4;
            border: 1px solid #bbf7d0;
            border-left: 4px solid #15803d;
            border-radius: 0 14px 14px 0;
            padding: 16px 20px;
            margin-bottom: 24px;
        }

        .info-box p {
            font-size: 14px;
            color: #14532d;
            line-height: 1.75;
        }

        .closing {
            font-size: 13.5px;
            color: #6b7280;
            line-height: 1.75;
            padding: 18px 20px;
            border-radius: 12px;
            background: #f9fafb;
            border: 1px solid #f3f4f6;
        }

        .closing a {
            color: #15803d;
            font-weight: 700;
            text-decoration: none;
        }

        .section-divider {
            height: 1px;
            background: #e8f5e9;
            margin: 0 40px;
        }

        .footer {
            background: #edf2e9;
            border-top: 1px solid #dde8da;
            padding: 22px 40px;
            text-align: center;
        }

        .footer-brand {
            display: flex;
            align-items: center;
            justify-content: center;
            gap: 8px;
            margin-bottom: 10px;
        }

        .footer-logo {
            width: 22px;
            height: 22px;
            border-radius: 5px;
            object-fit: contain;
        }

        .footer-brand-name {
            font-size: 12px;
            font-weight: 700;
            color: #4b5563;
        }

        .footer p {
            font-size: 11px;
            color: #9ca3af;
            line-height: 1.7;
        }

        @media only screen and (max-width: 620px) {
            .email-wrapper {
                margin: 16px !important;
                border-radius: 16px !important;
            }

            .header {
                padding: 28px 24px 24px !important;
            }

            .body {
                padding: 24px 24px 28px !important;
            }

            .section-divider {
                margin: 0 24px !important;
            }

            .footer {
                padding: 18px 24px !important;
            }
        }
    </style>
</head>

<body>
    <div class="email-wrapper">

        <div class="header">
            <div class="header-top">
                <div class="logo-box">
                    <img src="{{ asset('MI-Terpadu-Ibnu-Sina-Kembang-Jepara-Logo.png') }}" alt="MI Terpadu Ibnu Sina"
                        width="44" height="44">
                </div>
                <div>
                    <div class="header-school-name">
                        MI Terpadu Ibnu Sina
                        <small class="header-school-sub">Madrasah Ibtidaiyah &middot; Kembang, Jepara</small>
                    </div>
                </div>
            </div>
            <div class="header-divider"></div>
            <h1>Pertanyaan Diterima</h1>
            <p class="header-subtitle">Kami telah menerima pertanyaan Anda dan akan segera menjawabnya.</p>
            <div class="status-badge">
                <span class="badge-dot"></span>
                Dalam Proses Peninjauan
            </div>
        </div>

        <div class="body">
            <p class="greeting">
                Assalamu&#8217;alaikum, <strong>{{ $consultation->name }}</strong>.<br>
                Terima kasih telah menghubungi MI Terpadu Ibnu Sina. Pertanyaan Anda telah kami terima
                dan akan dijawab dalam <strong>1&ndash;2 hari kerja</strong> ke email ini.
            </p>

            <p class="section-label">Pertanyaan yang Anda Kirimkan</p>

            @if ($consultation->subject)
                <p class="question-subject">{{ $consultation->subject }}</p>
            @endif

            <div class="question-box">
                <p>{{ $consultation->message }}</p>
            </div>

            <div class="info-box">
                <p>
                    &#128276; <strong>Harap simpan email ini</strong> sebagai bukti bahwa pertanyaan Anda
                    telah diterima. Jawaban akan dikirimkan ke alamat email ini secara langsung.
                </p>
            </div>

            <div class="closing">
                Jika Anda memiliki pertanyaan mendesak, silakan hubungi kami langsung melalui WhatsApp
                atau kunjungi halaman
                <a href="{{ config('app.url') }}/konsultasi">Konsultasi</a> di website kami.<br><br>
                <em>Wassalamu&#8217;alaikum warahmatullahi wabarakatuh.</em>
            </div>
        </div>

        <div class="section-divider"></div>

        <div class="footer">
            <div class="footer-brand">
                <img src="{{ asset('MI-Terpadu-Ibnu-Sina-Kembang-Jepara-Logo.png') }}" alt="MI Terpadu Ibnu Sina"
                    class="footer-logo" width="22" height="22">
                <span class="footer-brand-name">MI Terpadu Ibnu Sina</span>
            </div>
            <p>
                Email ini dikirim otomatis oleh sistem informasi sekolah.<br>
                Jl. Raya Bangsri &ndash; Keling KM.4, Desa Jinggotan, Kec. Kembang, Kab. Jepara 59457
            </p>
        </div>

    </div>
</body>

</html>
